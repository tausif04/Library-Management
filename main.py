...
class Management(ABC):
    def __init__(self, root):
        ...
        self.all_book = Button(self.frame_2, text='All Books', font=(cs.font_1, 12), bd=2, command=self.ShowBooks,cursor="hand2", bg=cs.color_1,fg=cs.color_3).place(x=50,y=220,width=100)
        ...
        self.exit = Button(self.frame_2, text='Exit', font=(cs.font_1, 12), bd=2, command=self.Exit,cursor="hand2", bg=cs.color_5,fg=cs.color_3).place(x=180,y=220,width=100)


    def OnSelectedforReturn(self, a):
        self.dlt_record = Button(self.frame_3, text='Return', font=(cs.font_1, 12), bd=2, command=self.ReturningBook,cursor="hand2", bg=cs.color_2,fg=cs.color_3).place(x=50,y=0,width=100)
    ...
    def GetData_for_IssueBook(self):
        ...
        student_roll = Label(self.frame_1, text="Student Id", font=(cs.font_2, 15, "bold"), bg=cs.color_1).place(x=130,y=100)
        ...
        course = Label(self.frame_1, text="Department", font=(cs.font_2, 15, "bold"), bg=cs.color_1).place(x=130,y=170)
        self.course_entry = Entry(self.frame_1, bg=cs.color_4, fg=cs.color_3)
        self.course_entry.place(x=130,y=200, width=200)

        subject = Label(self.frame_1, text="Course Name", font=(cs.font_2, 15, "bold"), bg=cs.color_1).place(x=400,y=170)
    ...
    def GetBookNametoSearch(self):
        ...
        book_name = Label(self.frame_1, text="Enter the Book Id", font=(cs.font_2, 15, "bold"), bg=cs.color_1).place(x=220,y=140)
    ...
    def ReturnBook(self):
        self.ClearScreen()
        return_book = Label(self.frame_1, text="Return Book", font=(cs.font_1, 30, "bold"), bg=cs.color_4).place(x=250, y=40)
        roll_no = Label(self.frame_1, text="Enter the Student_Id.", font=(cs.font_2, 15, "bold"), bg=cs.color_1).place(x=210,y=140)
        self.roll_no_entry = Entry(self.frame_1, bg=cs.color_4, fg=cs.color_3)
        self.roll_no_entry.place(x=210,y=175, width=300)
    ...
    def ShowRecordsforReturn(self):
        if self.roll_no_entry.get() == "":
            ...
            try:
                ...
                if len(rows) == 0:
                    messagebox.showerror("Error!","This ID no. doesn't exists",parent=self.window)
                    connection.close()
                    ...
                    self.tree_1.heading('student_roll', text='Student Id', anchor=W)
                    self.tree_1.heading('student_name', text='Student Name', anchor=W)
                    self.tree_1.heading('course', text='Depertment', anchor=W)
                    self.tree_1.heading('subject', text='Course', anchor=W)
    ...
    def SearchBook(self):
        if self.book_entry.get() == "":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                curs = connection.cursor()
                curs.execute("select * from book_list where book_id like %s", ("%" + self.book_entry.get() + "%"))
                rows=curs.fetchall()
                if rows != None:
                    connection.close()
                else:
                    messagebox.showinfo("Database Empty", "This book is not available", parent=self.window)
                    connection.close()
                    self.ClearScreen()
